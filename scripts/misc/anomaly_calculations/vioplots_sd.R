...
# rcp 2.6
anomaly_model_2.6 <- anom_midcentury_2.6$sd_model
anomaly_gcm_2.6 <- anom_midcentury_2.6$sd_gcm
...
#### rcp 6.0 
anomaly_model_6.0 <- anom_midcentury_6.0$sd_model
anomaly_gcm_6.0 <- anom_midcentury_6.0$sd_gcm



#### rcp 8.5

anomaly_model_8.5 <- as.data.frame(anom_midcentury_8.5$sd_model)
anomaly_gcm_8.5 <- as.data.frame(anom_midcentury_8.5$sd_gcm)
...
anomaly_model_8.5_end <- as.data.frame(anom_endcentury_8.5$sd_model)
anomaly_gcm_8.5_end <- as.data.frame(anom_endcentury_8.5$sd_gcm)
...
ggplot(sub) +
  geom_line(aes(year, sd_gcm, color = model)) +
  facet_wrap(~gcm)

ggplot(sub) +
  geom_line(aes(year, sd_model, color = gcm)) +
  facet_wrap(~model)
...
ggsave('~/Dropbox/sundox/plots/vioplots_sd.png', dpi = 300,width = 384,height = 280, units = 'mm')
