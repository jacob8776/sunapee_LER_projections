...
library(dplyr)
library(lubridate)
setwd("~/Dropbox/sunapee_LER_projections/")
sim_folder <- getwd()
...
destination <- paste0(getwd()) # some location on your computer
try(download.file(data,destfile = paste0(destination, '/LER_inputs/2007-2020_buoy_templine_v26Feb2021.csv'),method="curl"))

field_temp_all <- file.path(paste0(sim_folder, '/LER_inputs/2007-2020_buoy_templine_v26Feb2021.csv'))
...
write.csv(field_format, row.names = FALSE, 'LER_inputs/field_temp_noon_obs.csv')
