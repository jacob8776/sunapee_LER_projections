...
fils <- list.files("IPSL-CM5A-LR", full.names = TRUE)
fils

both <- as.data.frame(seq.Date(from = as.Date("2006-01-01"), by = "day", length.out = 107381))


for(value in seq(14)){
  flist <- list.files("IPSL-CM5A-LR", full.names = TRUE, pattern = "rcp85")
  flist
...
write.csv(both, "IPSL_rcp85.csv", row.names = FALSE)
