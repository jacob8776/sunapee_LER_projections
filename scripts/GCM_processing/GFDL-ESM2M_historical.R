...
fils <- list.files("GFDL-ESM2M", full.names = TRUE)


both <- as.data.frame(seq.Date(from = as.Date("1661-01-01"), by = "day", length.out = 160341))


for(value in seq(14)){
  flist <- list.files("GFDL-ESM2M", full.names = TRUE, pattern = "piControl", )
  flist
...
write.csv(both, "GFDL_piControl.csv", row.names = FALSE)
