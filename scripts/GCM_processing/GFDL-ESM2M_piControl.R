...
both <- as.data.frame(seq.Date(from = as.Date("1661-01-01"), by = "day", length.out = 233389))


for(value in seq(14)){
  flist <- list.files("HadGEM2-ES", full.names = TRUE, pattern = "piControl", )
  flist
...
write.csv(both, "HADGEM2_piControl.csv", row.names = FALSE)
