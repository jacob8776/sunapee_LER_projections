...
fils <- list.files("HadGEM2-ES", full.names = TRUE)
fils

both <- as.data.frame(seq.Date(from = as.Date("2006-01-01"), by = "day", length.out = 34333))


for(value in seq(14)){
  flist <- list.files("HadGEM2-ES", full.names = TRUE, pattern = "rcp85")
  flist
...
write.csv(both, "hadGEM_rcp85.csv", row.names = FALSE)
